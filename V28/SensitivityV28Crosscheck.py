...
#print "Freq [GHz] , p_opt, p_cmb , p_hwp, p_apt, p_L1, p_L2, p_fil, p_feed, p_det"
    ...
    for j in range(0,n1):
        ...
        for k in range(0,num):
           ...
           # sm_emiss = sm_emiss_ref
            ...
            #if freq==freqLFT[i][j]:
               # print round(freq,2),", ",eff," , ",hwp_eff," , ",spill_5Kenve," , ",spill_5Khood," , ",spill_20K," , ",apt_eff,", ",pm_eff,", ",sm_eff,", ",fil_eff,", ",len_eff,", ",det_eff_LFT[i][j],", ",pol_hwp_LFT[i][j],", ",pol_dil_LFT[i][j]
               # print round(freq,2),", ",p_opt,", ",p_cmb,", ",p_hwp,", ",p_5Khood,", ",p_pm,", ",p_sm,", ",p_5Kenve," , ",p_20K," , ",p_fil,", ",p_len,", " ,p_det
           ...
           # p_2K  = f.BB(freq*1.e9,2)*spill_2K*fil_eff*len_eff*det_eff_LFT
           # p_5K  = f.BB(freq*1.e9,5)*spill_5K*fil_eff*len_eff*det_eff_LFT
           # p_20K = f.BB(freq*1.e9,20)*spill_20K*fil_eff*len_eff*det_eff_LFT
           
           # p_opt = p_cmb + p_hwp +  p_pm + p_sm + p_fil + p_len + p_2K + p_5K + p_20K
        ...
        NETarrLFTmargin[i][j] = NETdet/np.sqrt(2.*npixLFT[i][j]*0.8)  

        print round(freqLFT[i][j],2)," , ",round(Popt,8)," , ",round(NEPopt,8)," , ",round(NEPth,8)," , ",round(NEPread,8)," , ",round(NEPint,8)," , ",round(NEPext,8)," , ",round(NEPdet,8)," , ",round(NETdet,8)," , ",round(NETarrLFTmargin[i][j],8)
        ...
        for k in range(0,num):
           ...
           # if freq==freqHFT[i][j]:
            ...
            eff = hwp_eff*apt_eff*pm_eff*sm_eff*fil_eff*len_eff*det_eff_HFT[i][j]*pol_hwp_HFT[i][j]*pol_dil_HFT[i][j]
             ...
             #   print round(freq,2),", ",p_opt,", ",p_cmb,", ",p_hwp,", ",p_apt,", ",p_pm,", ",p_sm,", ",p_fil,", ",p_len,", " ,p_det
