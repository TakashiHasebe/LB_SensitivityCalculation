...
############################# Optics Temperatures ################################
T_bath,T_cmb, T_hwp_LFT,T_hwp_HFT, T_apt, T_mir, T_fil, T_len, T_lens, T_baf, Tr_hwp,Tr_mir,Tr_fil,Tr_len,Tr_lens,Tr_horn,Tr_det = op.Temp_Opt()
T_len = T_bath
T_horn = T_bath
Tr_len = T_baf
#Tr_horn = T_baf
    ...
    for j in range(0,n1):
        ...
        for k in range(0,num):
           # freq = freq_l+(freq_h - freq_l)*k/(num-1)
            ...
            eff_2K, spill_2K = f.Aperture(dpixLFT[i][j]*1e-3,bf_LFT,F_LFT,freq*1.e9)-apt_eff
            ...
            p_cmb = f.BB(freq*1.e9,T_cmb)*eff_hwp_LFT[i][j]*apt_eff*pm_eff*sm_eff*fil_eff*len_eff*det_eff_LFT[i][j]
           # p_2K = f.BB(freq*1.e9,T_apt)*spill_2K*hwp_eff*pm_eff*sm_eff*fil_eff*len_eff*det_eff_LFT[i][j]
            
            p_hwp = f.BB(freq*1.e9,T_hwp_LFT)*emiss_hwp_LFT[i][j]*apt_eff*pm_eff*sm_eff*fil_eff*det_eff_LFT[i][j]
            p_hwp_ref = f.BB(freq*1.e9,Tr_hwp)*ref_hwp_LFT[i][j]*apt_eff*pm_eff*sm_eff*fil_eff*det_eff_LFT[i][j]
            p_hwp = p_hwp + p_hwp_ref
            
            p_5Khood = f.BB(freq*1.e9,T_baf)*spill_5Khood*pm_eff*sm_eff*fil_eff*det_eff_LFT[i][j]
            ...
            p_2K = f.BB(freq*1.e9,T_apt)*spill_2K*fil_eff*det_eff_LFT[i][j]
            p_5Kenve =  f.BB(freq*1.e9,T_baf)*spill_5Kenve*fil_eff*det_eff_LFT[i][j]
            p_20K =  f.BB(freq*1.e9,T_hwp_LFT)*spill_20K*fil_eff*det_eff_LFT[i][j]
            ...
            p_len = f.BB(freq*1.e9,T_len)*det_eff_LFT[i][j]
            ...
            p_opt = p_cmb + p_2K + p_hwp + p_5Khood + p_pm + p_sm + p_5Kenve + p_20K + p_fil + p_det 

            eff =eff_hwp_LFT[i][j]*apt_eff*pm_eff*sm_eff*fil_eff*det_eff_LFT[i][j]*pol_hwp_LFT[i][j]*pol_dil_LFT[i][j]
            ...
            #if(freq == freqLFT[i][j]):
             #   print round(freqLFT[i][j],2)," , ",round(spill_2K,3)," , ",round(spill_5Kenve,3)," , ",round(spill_5Khood,3)," , ",round(spill_20K,3)," , ",round(apt_eff,3)
...
for i in range(0,m2):
    for j in range(0,n2):
        ...
        for k in range(0,num):
            ...
            p_pm_ref = f.BB(freq*1.e9,Tr_mir)*ref_L1[i][j]*sm_eff*len_eff*fil_eff*det_eff_HFT[i][j]
            p_pm = p_pm + p_pm_ref
            ...
            p_len_ref = f.BB(freq*1.e9,Tr_horn)*ref_len*det_eff_HFT[i][j]
            p_len = p_len + p_len_ref
