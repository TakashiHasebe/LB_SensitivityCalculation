...
#def p_cmb = f.BB(freq*1.e9,T_cmb)*hwp_eff*apt_eff*pm_eff*sm_eff*fil_eff*len_eff*det_eff_LFT
...
for i in range(0,m2):
    for j in range(0,n2):
        ...
        for k in range(0,num):
           ...
           # if freq==freqHFT[i][j]:
            ...
            if freq==freqHFT[i][j]:
               ...
               # print round(freq,2),"hwp_eff, ",hwp_eff,"apt_eff, ",apt_eff,"pm_eff, ",pm_eff,"sm_eff, ",sm_eff,"fil_eff, ",fil_eff,"len_eff, ",len_eff,"det_eff, ",det_eff_HFT[i][j]
               # print round(freq,2),"hwp_emiss, ",hwp_emiss_HFT[i][j],"apt_emiss, ",apt_emiss,"lens1_emiss, ",pm_emiss,"lens2_eff, ",sm_emiss,"2Kfilter_emiss, ",fil_emiss,"lenslet_emiss, ",len_emiss
               # print "spillover eff =",apt_emiss
           # print "m= ",m2,"n1= ",n2,"k= ",k,"freq= ",freq
            ...
            p_cmb_arr[k]= p_cmb
            if k ==0:
                p_cmb_sum[k]=p_cmb_arr[k]
            else:
                p_cmb_sum[k]=p_cmb_arr[k-1]+p_cmb_arr[k]
        ...
        Popt = np.sum(p_opt_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        #Popt = integrate.quad(p_opt,freq_l,freq_h)
        #Pcmb = integrate.quad(fp_cmb,freq_l*1.e9,freq_h*1.e9)
        #Pcmb = integrate.quad(fp_cmb,freq_l,freq_h)
        Pcmb = np.sum(p_cmb_sum)*(freq_h-freq_l)*1.e9/num*1.e12/2 # in unit of pW
        Phwp = np.sum(p_hwp_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        Papt = np.sum(p_apt_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        Ppm = np.sum(p_pm_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        Psm = np.sum(p_sm_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        Pfil = np.sum(p_fil_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        #Plen = np.sum(p_len_arr)*(freq_h-freq_l)*1.e9/num*1.e12 # in unit of pW
        Plen = integrate.quad(fp_len,freq_l,freq_h)
       ...
       # print round(freqHFT[i][j],2)," , ",round(npixHFT[i][j],0)," , ",round(Popt,8)," , ",round(NEPopt,8)," , ",round(NEPth,3)," , ",round(NEPread,3)," , ",round(NEPint,2)," , ",round(NETdet,2)," , ",round(NETarrHFT[i][j],2)," , ",round(NETarrHFTmargin[i][j],2)
        ...
        print round(freqHFT[i][j],2)," , ",Pcmb," , ",Phwp," , ",Papt," , ",Ppm," , ",Psm," , ",Pfil," , ",Plen," , ",Popt
