...
################################# MFT Spill 5K/2K parameters ##########################################
   # apt_spill_MFT = np.array([0.102, 0.047, 0.017, 0.005, 0.002])
    apt_spill_MFT = np.array([2.35e-2, 1.19e-2, 5.01e-3, 5.11e-3, 4.89e-3])
    spill_5K_MFT = np.array([6.32e-2, 6.25e-2, 6.10e-2, 6.30e-2, 6.20e-2])
    spill_2K_MFT = np.array([6.33e-2, 6.24e-2, 6.16e-2, 6.56e-2, 6.79e-2])    
    sky_eff_MFT = np.array([6.73e-1, 7.18e-1, 7.20e-1, 6.75e-1, 6.06e-1])

    ref1_5K_MFT = np.array([2.44e-3, 5.00e-4, 2.70e-7, 3.24e-4, 8.48e-4])# 1st order reflection terminated to 5K
    ref1_2K_MFT = np.array([7.74e-4, 3.81e-4, 3.54e-4, 8.78e-4, 1.76e-3])# 1st order reflection terminated to 2K
    ref1_FP_MFT = np.array([3.08e-2, 9.20e-3, 1.40e-4, 1.45e-2, 5.23e-2])# 1st order reflection terminated to FP

    ref2_sky_MFT = np.array([3.96e-4, 3.73e-5, 5.39e-11, 8.76e-5, 1.22e-3])# 2nd order reflection goes to sky
    ref2_5K_MFT = np.array([1.40e-4, 2.33e-5, 1.27e-8, 5.23e-5, 3.85e-4])# 2nd order reflection terminated to 5K
    ref2_2K_MFT = np.array([1.93e-5, 1.44e-6, 8.88e-11, 3.04e-6, 3.54e-5])# 2nd order reflection terminated to 2K
    ref2_apt_MFT = np.array([6.33e-5, 4.33e-6, 3.38e-11, 7.41e-6, 8.86e-5])# 2nd order reflection terminated to stop
