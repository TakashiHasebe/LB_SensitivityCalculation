...
################################# MFT Spill 5K/2K parameters ##########################################
   # apt_spill_MFT = np.array([0.102, 0.047, 0.017, 0.005, 0.002])
    apt_spill_MFT = np.array([2.57e-2, 1.32e-2, 5.66e-3, 5.90e-3, 5.79e-3])
    spill_5K_MFT = np.array([6.56e-2, 6.53e-2, 6.43e-2, 6.70e-2, 6.67e-2])
    spill_2K_MFT = np.array([6.33e-2, 6.24e-2, 6.16e-2, 6.56e-2, 6.79e-2])    
    sky_eff_MFT = np.array([7.50e-1, 8.18e-1, 8.42e-1, 8.13e-1, 7.55e-1])

    ref1_5K_MFT = np.array([2.80e-3, 5.93e-4, 4.12e-7, 4.32e-4, 1.18e-3])# 1st order reflection terminated to 5K
    ref1_2K_MFT = np.array([8.68e-4, 4.14e-4, 3.55e-4, 9.37e-4, 2.02e-3])# 1st order reflection terminated to 2K
    ref1_FP_MFT = np.array([3.52e-2, 1.09e-2, 1.41e-4, 1.81e-2, 6.76e-2])# 1st order reflection terminated to FP

    ref2_sky_MFT = np.array([4.89e-4, 4.81e-5, 6.91e-11, 1.25e-4, 1.85e-3])# 2nd order reflection goes to sky
    ref2_5K_MFT = np.array([1.58e-4, 2.76e-5, 1.50e-8, 6.58e-5, 4.93e-4])# 2nd order reflection terminated to 5K
    ref2_2K_MFT = np.array([2.25e-5, 1.72e-6, 1.11e-10, 3.84e-6, 4.54e-5])# 2nd order reflection terminated to 2K
    ref2_apt_MFT = np.array([7.62e-5, 5.34e-6, 4.12e-11, 9.53e-6, 1.18e-4])# 2nd order reflection terminated to stop
