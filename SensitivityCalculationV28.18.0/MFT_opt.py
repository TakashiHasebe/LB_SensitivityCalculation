...
################################# MFT Detectors parameters ##########################################
    ...
    dpix_MFT = np.array([10., 12., 10.,12., 10.])
    npix_MFT = np.array([273.,244.,273.,244.,273])
...
################################# MFT Spill 5K/2K parameters ##########################################
   # apt_spill_MFT = np.array([0.102, 0.047, 0.017, 0.005, 0.002])
    apt_spill_MFT = np.array([1.33e-1, 3.75e-2, 4.10e-2, 2.69e-2, 2.62e-2])
    spill_5K_MFT = np.array([7.23e-2, 6.27e-2, 6.31e-2, 6.07e-2, 5.93e-2])
    spill_2K_MFT = np.array([6.52e-2, 6.32e-2, 6.33e-2, 6.16e-2, 6.17e-2])    
    sky_eff_MFT = np.array([5.94e-1, 7.22e-1, 7.35e-1, 7.49e-1, 7.16e-1])

    ref1_5K_MFT = np.array([6.14e-3, 1.44e-3, 2.53e-4, 6.20e-5, 8.25e-4])# 1st order reflection terminated to 5K
    ref1_2K_MFT = np.array([2.95e-3, 2.06e-3, 1.55e-3, 1.81e-3, 2.96e-3])# 1st order reflection terminated to 2K
    ref1_FP_MFT = np.array([4.43e-2, 2.37e-2, 4.52e-3, 2.26e-3, 2.81e-2])# 1st order reflection terminated to FP

    ref2_sky_MFT = np.array([7.52e-4, 2.27e-4, 8.45e-6, 1.66e-6, 3.12e-4])# 2nd order reflection goes to sky
    ref2_5K_MFT = np.array([3.23e-4, 9.16e-5, 1.08e-5, 4.08e-6, 1.46e-4])# 2nd order reflection terminated to 5K
    ref2_2K_MFT = np.array([0, 0, 0, 0, 0])# 2nd order reflection terminated to 2K
    ref2_apt_MFT = np.array([1.52e-4, 2.77e-5, 8.88e-7, 1.52e-7, 2.58e-5])# 2nd order reflection terminated to stop
