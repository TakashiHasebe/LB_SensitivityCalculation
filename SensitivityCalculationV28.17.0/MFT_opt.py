...
################################# MFT Detectors parameters ##########################################
    ...
    dpix_MFT = np.array([12., 10., 12.,10., 12.])
    npix_MFT = np.array([183.,364.,183.,364.,183])
...
################################# MFT Spill 5K/2K parameters ##########################################
   # apt_spill_MFT = np.array([0.102, 0.047, 0.017, 0.005, 0.002])
    apt_spill_MFT = np.array([7.29e-2, 7.72e-2, 2.75e-2, 2.75e-2, 2.56e-2])
    spill_5K_MFT = np.array([6.28e-2, 6.36e-2, 6.22e-2, 6.17e-2, 5.83e-2])
    spill_2K_MFT = np.array([6.41e-2, 6.42e-2, 6.24e-2, 6.24e-2, 6.10e-2])    
    sky_eff_MFT = np.array([6.60e-1, 6.82e-1, 7.49e-1, 7.47e-1, 7.17e-1])

    ref1_5K_MFT = np.array([4.56e-3, 2.08e-3, 1.84e-4, 1.11e-4, 5.68e-4])# 1st order reflection terminated to 5K
    ref1_2K_MFT = np.array([2.97e-3, 2.09e-3, 1.51e-3, 1.89e-3, 2.74e-3])# 1st order reflection terminated to 2K
    ref1_FP_MFT = np.array([4.43e-2, 2.24e-2, 4.52e-3, 2.32e-3, 2.81e-2])# 1st order reflection terminated to FP

    ref2_sky_MFT = np.array([8.92e-4, 1.98e-4, 9.42e-6, 1.63e-6, 3.21e-4])# 2nd order reflection goes to sky
    ref2_5K_MFT = np.array([3.29e-4, 9.35e-5, 1.01e-5, 6.02e-6, 1.05e-4])# 2nd order reflection terminated to 5K
    ref2_2K_MFT = np.array([0, 0, 0, 0, 0])# 2nd order reflection terminated to 2K
    ref2_apt_MFT = np.array([1.48e-4, 3.09e-5, 7.64e-7, 2.71e-7, 1.92e-5])# 2nd order reflection terminated to stop
