...
def LFT_MHFT_sensitivity_calculator(Freq=Freq, DC=DC, CR =CR, CT=CT, df_LFT=df_LFT, df_HFT=df_HFT, T_bath=T_bath, T_baf=T_baf, T_fil=T_fil, T_apt_LFT=T_apt_LFT, T_apt_HFT=T_apt_HFT, T_hwp_LFT=T_hwp_LFT, T_hwp_HFT=T_hwp_HFT, det_eff_LFT=det_eff_LFT,  det_eff_HFT=det_eff_HFT):
    ...
    for i in range(0,m1):
        for j in range(0,n1):
            ...
            for k in range(0,num):
                ...
                p_apt = f.BB(freq*1.e9,T_apt_LFT)*spill_apt*fil_eff*len_eff*det_eff_LFT[i][j]
                ...
                eff = hwp_eff*apt_eff*pm_eff*sm_eff*fil_eff*len_eff*det_eff_LFT[i][j]*pol_hwp_LFT[i][j]*df_LFT[i][j]
                p_opt_arr[k] = p_opt
                p_cmb_arr[k] = p_cmb
                p_hwp_arr[k] = p_hwp
                p_apt_arr[k] = p_apt
