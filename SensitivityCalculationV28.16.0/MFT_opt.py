...
################################# MFT Detectors parameters ##########################################
    ...
    dpix_MFT = np.array([10., 10., 10.,10., 10.])
    npix_MFT = np.array([273.,364.,273.,364.,273])
...
################################# MFT Spill 5K/2K parameters ##########################################
   # apt_spill_MFT = np.array([0.102, 0.047, 0.017, 0.005, 0.002])
    apt_spill_MFT = np.array([1.33e-1, 7.72e-2, 4.10e-2, 2.75e-2, 2.62e-2])
    spill_5K_MFT = np.array([7.23e-2, 6.36e-2, 6.31e-2, 6.17e-2, 5.93e-2])
    spill_2K_MFT = np.array([6.52e-2, 6.42e-2, 6.33e-2, 6.24e-2, 6.17e-2])    
    sky_eff_MFT = np.array([5.94e-1, 6.82e-1, 7.35e-1, 7.47e-1, 7.16e-1])

    ref1_5K_MFT = np.array([6.14e-3, 2.08e-3, 2.53e-4, 1.11e-4, 8.25e-4])# 1st order reflection terminated to 5K
    ref1_2K_MFT = np.array([2.95e-3, 2.09e-3, 1.55e-3, 1.89e-3, 2.96e-3])# 1st order reflection terminated to 2K
    ref1_FP_MFT = np.array([4.43e-2, 2.24e-2, 4.52e-3, 2.32e-3, 2.81e-2])# 1st order reflection terminated to FP

    ref2_sky_MFT = np.array([7.52e-4, 1.98e-4, 8.45e-6, 1.63e-6, 3.12e-4])# 2nd order reflection goes to sky
    ref2_5K_MFT = np.array([3.23e-4, 9.35e-5, 1.08e-5, 6.02e-6, 1.46e-4])# 2nd order reflection terminated to 5K
    ref2_2K_MFT = np.array([0, 0, 0, 0, 0])# 2nd order reflection terminated to 2K
    ref2_apt_MFT = np.array([1.52e-4, 3.09e-5, 8.88e-7, 2.71e-7, 2.58e-5])# 2nd order reflection terminated to stop
